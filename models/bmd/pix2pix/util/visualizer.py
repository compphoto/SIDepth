...
import numpy as np
import torch
from dependencies.bmd.pix2pix.util import html, util
...
class Visualizer():
    ...
    def display_current_results(self, visuals, epoch, save_result, outer_activation):
        ...
        if self.use_html and (save_result or not self.saved):
            ...
            for label, image in visuals.items():
                if 'surfnorm' in label:
                    image = (image + 1) / 2
                image_numpy = util.tensor2im(image, outer_activation=outer_activation)
    ...
    def print_current_losses(self, epoch, iters, total_iters, losses, t_comp, t_data):
        ...
        """
        message = '(epoch: %d, iters: %d/%d, time: %.3f, data: %.3f) ' % (epoch, iters, total_iters, t_comp, t_data)
