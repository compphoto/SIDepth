...
class BaseModel(ABC):
    ...
    def setup(self, opt):
        ...
        if opt.ordinal_weights is not None:
            # load generator weights from ordinal weights
            print(f"Loading ordinal weights from {opt.ordinal_weights}")
            self.load_ordinal_g_weights(opt)
    ...
    def load_o2m_weights(self, weights):
        """Load generator ordinal weights
        ...
        for name in model_names:
            if isinstance(name, str):
                print(f"Loading {name} from {weights}")
                net = getattr(self, 'net' + name)
                if isinstance(net, torch.nn.DataParallel):
                    net = net.module
                ...
                # GitHub source), you can remove str() on self.device
                state_dict = torch.load(
                    weights, map_location=str(self.device))
                if hasattr(state_dict, '_metadata'):
                    del state_dict._metadata
    ...
    def load_ordinal_g_weights(self, opt):
        """Load generator ordinal weights
        ...
        for name in model_names:
            if isinstance(name, str):
                load_path = opt.ordinal_weights.replace('_G', f'_{name}')
