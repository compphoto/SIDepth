...
def _make_encoder(backbone, features, use_pretrained, groups=1, expand=False, exportable=True, hooks=None,
    ...
    if backbone == "beitl16_512":
        ...
        pretrained = _make_pretrained_efficientnet_l2(use_pretrained, exportable=exportable)
        scratch = _make_scratch([104, 176, 480, 1376], features, groups=groups, expand=expand)  # _make_pretrained_efficientnet_l2
    elif backbone == "efficientnet_b7":
        pretrained = _make_pretrained_efficientnet_b7(use_pretrained, exportable=exportable)
        scratch = _make_scratch([48, 80, 224, 640], features, groups=groups, expand=expand)  # _make_pretrained_efficientnet_l2
    else:
...
def _make_pretrained_efficientnet_l2(use_pretrained, exportable=False):
    efficientnet = torch.hub.load(
        "rwightman/gen-efficientnet-pytorch",
...
def _make_pretrained_efficientnet_b7(use_pretrained, exportable=False):
    efficientnet = torch.hub.load(
        "rwightman/gen-efficientnet-pytorch",
