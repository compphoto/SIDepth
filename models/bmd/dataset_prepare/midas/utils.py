...
# def write_depth(path, depth, bits=1, colorize=False):
...
def write_depth(path, depth, bits=1, colorize=False, save_raw=False):
    """Write depth map to pfm and png file.
    ...
    if save_raw:
        save_raw_float32_image("{}.raw".format(path), depth)
...
def save_raw_float32_image(file_name: str, image: np.ndarray):
    """Method to save the image in .raw format 

    Args:
        file_name (str): path of the output file
