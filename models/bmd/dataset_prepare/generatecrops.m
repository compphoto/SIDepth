function generatecrops(dataset, blsize, stride, subsetname, counter_offset)
    ...
    % root_dir = '/project/aksoy-lab/datasets/RGBD/unsplash/mergenet/';
    % root_dir = '/project/aksoy-lab/datasets/RGBD/unsplash/mergenet/test_data/'
    root_dir = '/project/aksoy-lab/datasets/RGBD/unsplash/mergenet/test_data_v2'
    ...
    imglist = dir(sprintf('%s/*.jpg', imgdir));

    if strcmp(dataset, 'unsplash')
        test_set_size = 5;
    else
        test_set_size = 0;
    end
    ...
    for i = 1 + i_start:numel(imglist) + i_end
        samplename = erase(imglist(i).name, '.jpg');

        img = im2uint16(imread(sprintf('%s/%s.jpg', imgdir, samplename)));
